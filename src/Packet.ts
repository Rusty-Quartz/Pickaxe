...
export const parsePackets = (dataDir: string, quartzDir: string) => {
	// Read in all packet info
	console.log('Loading protocol.json...');
	let packetInfo: State[] = require(`${dataDir}protocol.json`);


	// Read in all type mappings
	console.log('Loading in mappings.json...');
	let mappingsRaw:Mappings = require(`${dataDir}mappings.json`);
		...
		if(data[0] == asyncPackerHandlerIndex || data[0] == syncPacketHandlerIndex) {
			...
			fnBody.forEach((line, i) => {
				if(line.trim().startsWith('fn ')) {
					if(handlers.length == 0) throw new Error('UhOh there are more handlers in packet_handlers.rs than are loaded in from protocol.json, quitting to avoid messing up function bodies');
